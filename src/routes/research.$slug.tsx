...
export const Route = createFileRoute("/research/$slug")({
  head: ({ params }) => {
    ...
    const title = brief
      ? `${brief.title.slice(0, 60)}${brief.title.length > 60 ? "…" : ""} — Research · Elenchos`
      : "Research brief — Elenchos";
    const description =
      brief?.subtitle ??
      "Thesis-style research brief — method-first, separate from Topics.";
    const url = `https://elenchos.live/research/${params.slug}`;
    return {
      meta: [
        { title },
